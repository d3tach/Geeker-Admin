...
//数据更新
const switchPerfDailyFilterRef = ref();
const filterResult = ref();
const dataResult = ref();
let device_infos: any = {}; //设备信息存个变量,避免多次重复请求数据
...
const handleFilterChanged = (filter: any) => {
  ...
  if (switchPerfDailyFilterRef.value) {
    switchPerfDailyFilterRef.value.getTypeData();
  }
};
...
const deal_memory_data = (data: any) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    for (let j = 0; j < memory_param.length; j++) {
      ...
      const avg_memory = average(item.memory_data.map((memory: any) => memory[memoryParam].Pss));
      let legendName;
      if (filterResult.value["device_id"] && filterResult.value["device_id"].length === 1) {
...
const deal_cpu_freq = (data: any) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    for (let j = 0; j < device_infos[item.device_id].cpu_info.cpu_affected_lists.length; j++) {
      const avg_cpu_freq = average(item.cpu_data.cpu_freq.map((cpu_freq: any) => cpu_freq[j]));
      let legendName;
      if (filterResult.value["device_id"] && filterResult.value["device_id"].length === 1) {
